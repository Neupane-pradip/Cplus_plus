...
const string GREETING_AT_END = "Thanks and see you later!",
             PARA_ERROR = "Error: wrong number of parameters.",
             NUMBER_ERROR = "Error: a non-number operand.",
...
std::string stringupper(std::string const& str)
{
    std::string m = str;
    for (std::string::size_type i = 0; i < str.length(); i++)
    {
        m.at(i) = toupper(str.at(i));
    }
    return m;
}
int main() {
    ...
    while ( true ) {
        ...
        {
            std::string commandName = commands_iter->str;
            if (stringupper(command_to_be_executed) == commandName)
            {
                command_found = true;
