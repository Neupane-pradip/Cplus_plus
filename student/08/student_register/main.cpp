...
void modifyStudentPhoneNumber(Student* student) {
    std::cout << "Enter a new phone number: ";
    std::string newNumber;
    std::getline(std::cin, newNumber);
    std::cout << "\n";

    if(is_valid_phone_number(newNumber)) {
        student->phone_number = newNumber;
    }
}

void saveData(const std::map<std::string, Student*>& ids, const std::string& fileName) {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string student_number = parts.at(1);
                        ...
                        modifyStudentPhoneNumber(student_numbers.at(student_number));
